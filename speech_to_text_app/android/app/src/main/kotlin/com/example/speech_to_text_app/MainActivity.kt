...
class MainActivity: FlutterActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        Log.d("ZaloPay", "ZaloPaySDK initialized with AppID: 2554")
    }
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, channelPayOrder)
            .setMethodCallHandler { call, result ->
                if (call.method == "payOrder") {
                    ...
                    val tagCancel = "[onPaymentCancel]"
                    val token = call.argument<String>("zptoken")

                    if (token == null || token.isEmpty()) {
                        Log.d("ZaloPay", "zptoken is null or empty")
                        result.success("Payment failed")
                        return@setMethodCallHandler
                    }

                    Log.d("ZaloPay", "Starting payment with zptranstoken: $token")

                    ZaloPaySDK.getInstance().payOrder(this@MainActivity, token !!, "demozpdk://app",object: PayOrderListener {
                            override fun onPaymentCanceled(zpTransToken: String?, appTransID: String?) {
                                Log.d(tagCancel, String.format("[TransactionId]: %s, [appTransID]: %s", zpTransToken, appTransID))
